...
fn frame_size_error(hdr: &FrameHeader, t: FrameType) -> HeaderValidation {
    // §4.2 + per-frame rules: a frame-size error is a CONNECTION error except for PRIORITY (§6.3)
    // and DATA, which are stream errors when on a non-zero stream. In particular RST_STREAM (§6.4)
