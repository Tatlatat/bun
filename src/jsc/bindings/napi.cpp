    ...
    size_t* byte_offset)
{
    ...
    if (type) {
        ...
        // stops at `napi_biguint64_array`). Node leaves `*type` unwritten and still returns
        // `napi_ok` in that case (its type if/else-if chain simply has no matching arm), so we
        // match that: write the mapped value when there is one, otherwise leave `*type` as-is.
