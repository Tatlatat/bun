...
describe("cleanup hooks", () => {
  ...
  describe("napi_get_typedarray_info / napi_get_dataview_info", () => {
    it("reports the view's real byteOffset so that arraybuffer-base + byte_offset == data", async () => {
      ...
      // A Float16Array has no napi_typedarray_type value. It still succeeds: with
      // type=NULL, and with type=&out where `*type` is left unchanged (matches Node).
      ...
      if (float16) {
        ...
        expect(output).toContain("PASS: get_typedarray_info(float16, type=&out)");
      } else {
