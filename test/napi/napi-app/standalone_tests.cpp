...
test_napi_typedarray_dataview_byte_offset(const Napi::CallbackInfo &info) {
  ...
  if (!float16_supported) {
    ...
    // type!=NULL: N-API has no Float16 enum value, so `*type` is left as the
    // caller set it and the call still returns napi_ok (matches Node). Seed a
    // sentinel and confirm it's preserved.
    napi_typedarray_type f16_type = (napi_typedarray_type)0x7f;
    s = napi_get_typedarray_info(env, f16, &f16_type, &f16_len, &f16_data,
                                 nullptr, &f16_off);
    printf("%s: get_typedarray_info(float16, type=&out) status=%d type=%d\n",
           (s == napi_ok && f16_type == (napi_typedarray_type)0x7f) ? "PASS"
                                                                    : "FAIL",
           s, (int)f16_type);
  }
